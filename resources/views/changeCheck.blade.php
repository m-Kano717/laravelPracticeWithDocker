...
<body>
    ...
    @include("common.navi")
    <h3>会員情報確認</h3>
    <form action="/changeRegist" method="POST">
        <input type="hidden" name="first_name" value='{{ $userInfo["first_name"] ?? "" }}'>
    <input type="hidden" name="last_name" value='{{ $userInfo["last_name"] ?? "" }}'>
    <input type="hidden" name="pass" value='{{ $userInfo["pass"] ?? "" }}'>
    <input type="hidden" name="nick_name" value='{{ $userInfo["nick_name"] ?? "" }}'>
    ...
    <input type="hidden" name="birth_date" value='{{ $userInfo["birth_date"] ?? "" }}'>
    ...
    @csrf
    名前<input type="text" name="first_name" value='{{ $userInfo["first_name"] ?? "" }}' disabled>
        <input type="text" name="last_name" value='{{ $userInfo["last_name"] ?? "" }}' disabled><br/>
    パスワード：<input type="password" name="pass" value='{{ $userInfo["pass"] ?? "" }}' disabled> 
    ニックネーム<input type="text" name="nick_name" value='{{ $userInfo["nick_name"] ?? "" }}' disabled><br/>
    
    住所（都道府県）<select name="address1" disabled>
    ...
    住所（市区町村）<input type="text" name="address2" value='{{ $userInfo["address2"] ?? "" }}' disabled><br/>
    住所１<input type="text" name="address3" value='{{ $userInfo["address3"] ?? "" }}' disabled>
    住所２<input type="text" name="address4" value='{{ $userInfo["address4"] ?? "" }}' disabled><br/>
    生年月日<input type="date" name="birth_date" value='{{ $userInfo["birth_date"] ?? "" }}' disabled><br/>
    
    性別
        <label><input type="radio" name="sex" value="1" {{ ($userInfo['sex'] ?? '') == '1' ? 'checked' : '' }} disabled>男性</label>
        <label><input type="radio" name="sex" value="2" {{ ($userInfo['sex'] ?? '') == '2' ? 'checked' : '' }} disabled>女性</label>
        <label><input type="radio" name="sex" value="3" {{ ($userInfo['sex'] ?? '') == '3' ? 'checked' : '' }} disabled>その他</label><br/>
        
    電話番号<input type="tel" name="tel" value='{{ $userInfo["tel"] ?? "" }}' disabled>
    メールアドレス<input type="mail" name="mail" value="{{ $userInfo["mail"] }}" disabled><br/>
    
    会員タイプ
        <label><input type="radio" name="user_type" value="A" {{ ($userInfo['user_type'] ?? '') == 'A' ? 'checked' : '' }} disabled>A</label>
        <label><input type="radio" name="user_type" value="B" {{ ($userInfo['user_type'] ?? '') == 'B' ? 'checked' : '' }} disabled>B</label>
        <label><input type="radio" name="user_type" value="C" {{ ($userInfo['user_type'] ?? '') == 'C' ? 'checked' : '' }} disabled>C</label><br/>
